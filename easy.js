...
var twoSum = function (nums, target) {
  ...
  let recursiveCall = (nums, target) => {
    ...
    return (
      recursiveCall(leftHalf, target) ||
      recursiveCall(rightHalf, target) ||
      mergeCall(leftHalf, rightHalf, target)
    );
  };
  ...
  for (let i = 0; i < s.length; i++) {
    switch (s[i]) {
      ...
      case "}":
        ...
        if (
          s[i].charCodeAt(0) === stackTop.charCodeAt(0) + 1 ||
          s[i].charCodeAt(0) === stackTop.charCodeAt(0) + 2
        ) {
          continue;
        } else {
          return false;
...
var mergeTwoLists = function (list1, list2) {
  if (!list1 || !list1.hasOwnProperty("val")) {
    return list2;
  } else if (!list2 || !list2.hasOwnProperty("val")) {
    return list1;
  } else {
...
var removeElement = function (nums, val) {
  let currentElementPointer = 0;
  let lastElementPointer = nums.length - 1;
  while (currentElementPointer < nums.length) {
    console.log(currentElementPointer, lastElementPointer, nums);
    if (currentElementPointer == lastElementPointer) {
      if (nums[currentElementPointer] === val) {
      ...
      nums[currentElementPointer] === val &&
      nums[lastElementPointer] !== val
    ) {
      nums[currentElementPointer] = nums[lastElementPointer];
      lastElementPointer -= 1;
    } else if (
      nums[currentElementPointer] === val &&
      nums[lastElementPointer] === val
